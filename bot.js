...
const encouragements = require('./encouragements.json');
...
client.on('message', message => {
    ...
    if (message.content == commands[messageLanguage].prompt) {
     ...
     // Encouragements
    if (message.content == commands[messageLanguage].encouragement) {
        var themes = encouragements.writtingEncouragements;
        var randomNumberRaw = Math.floor(Math.random() * (themes.length));
        var thisEncouragement = themes[randomNumberRaw];
        message.reply(lang[messageLanguage].showEncouragement + thisEncouragement);
        logMessage("received encouragement number " + randomNumberRaw, message.author.username, guildprop.name);
    }
